...
class VisionModelTask(BaseTask):
    ...
    def execute(self, **kwargs) -> dict:
        ...
        Arguments:
            ...
            prompt: str
        """
        grounding_dino_bbox = self.detect_grounding_dino(kwargs["image"], kwargs["prompt"])
        if grounding_dino_bbox is None:
            return {'bboxs': [], 'overlay_images': []}
        bbox, overlay_image = self.detect_sam(
            kwargs["image"], grounding_dino_bbox
        )
        return {
            "bboxs": [bbox],
            "overlay_images": [overlay_image]
        }
